  ...
  Feature("Edit Accounting Period - Validations") {
    ...
    Scenario("Edit Accounting Period") {
      Then("the user is presented with a change link next to the accounting period dates")
        ...
        thenTheUserIsPresentedWithAChangeLinkNextToTheAccountingPeriodDates()
        // --- Other possible matches ---
        // thenTheUserClicksTheChangeLinkNextToTheAccountingPeriodDates() [0.79] (CheckYourAnswersStepDefSteps.scala) pattern: the user clicks the change link next to the accounting period dates
